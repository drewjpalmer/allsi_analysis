{
	...
	for (int i = 0; i < 6; i++){
		...
		for (int j = 0; j < 7; j++){
			...
			std12[i][j] = myfit1[i][j]->GetParameter(5);
			...
			{	
				...
				std1_err[i][j] = myfit1[i][j]->GetParError(5);
				sigma1[i]->SetBinError(j+1, std1_err[i][j]);
			}
			...
			std22[i][j] = myfit2[i][j]->GetParameter(5);
			...
			{	
				...
				std2_err[i][j] = myfit2[i][j]->GetParError(5);
				sigma2[i]->SetBinError(j+1, std2_err[i][j]);
			}
			...
			std32[i][j] = myfit3[i][j]->GetParameter(5);
			...
			{	
				...
				std3_err[i][j] = myfit3[i][j]->GetParError(5);
				sigma3[i]->SetBinError(j+1, std3_err[i][j]);
			}
			...
			std42[i][j] = myfit4[i][j]->GetParameter(5);
			...
			{	
				...
				std4_err[i][j] = myfit4[i][j]->GetParError(5);
				sigma4[i]->SetBinError(j+1, std4_err[i][j]);
			}
		...
		sigmab[i]->SetTitle(Form("3-vtx/2-vtx  p: %d - %d GeV;#eta;3-vtx / 2-vtx Longitudinal DCA Resolution", (i * 5), ((i * 5) + 5))); 
		...
		sigmas[i]->SetTitle(Form("3-vtx/2-vtx  p: %d - %d GeV;#eta;3-vtx / 2-vtx Longitudinal DCA Resolution", (i * 5), ((i * 5) + 5))); 
