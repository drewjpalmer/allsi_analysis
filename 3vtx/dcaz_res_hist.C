{
	...
	for (int j = 0; j < 8; j++)
	{
		...
		for (int i = 0; i < 6; i++){
			...
			fit[j][i]->SetParLimits(4, -10*j, 10*j);
			fit[j][i]->SetParLimits(0, hist[i][j]->GetMaximum()*.05, hist[i][j]->GetMaximum()*1.1);
			fit[j][i]->SetParLimits(3, hist[i][j]->GetMaximum()*.05, hist[i][j]->GetMaximum()*1.1);
			fit[j][i]->SetParLimits(2, fit_range*.5, 15000. );
			fit[j][i]->SetParLimits(5, fit_range*.2 ,fit_range*1.5);
			hist[i][j]->Fit("fit", "R");	
			hist[i][j]->Draw();
