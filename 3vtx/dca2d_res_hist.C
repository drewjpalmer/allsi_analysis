{
	...
	TString name1 = "out_simp_geom_3vtx_Beast_FastSimEval";
	TString str_input = name1 + ".root";
	TString str_out = "dca2d_output_" + name1 + ".root";
	...
	TH1F *dist = new TH1F("dca2d_dist", "Momentum Resolution distibution in momentum: p:counts", 50, -1000., 1000.);


	float test_axis = 200.; //set this and plot so all data are in range	
	...
	for (int i=0; i<6; i++){	
		...
		for (int j=0; j<8; j++){	
			...
			hist_range[i][j] = new TH1F(Form("h_range_%i_%i", i, j), Form("Transverse DCA Resolution. #eta: %.1f - %.1f p: %d - %d GeV;#mum;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 500, -test_axis*(j+1), test_axis*(j+1));
		}
	}
			...
			for (int j = 0; j < 8; j++)
			{
				if (eta >= (float)(j) / 2. && eta < (float)(j) / 2. + 0.5)
				{
					for (int i = 0; i < 6; i++)
					{
						...
						{
							hist_range[i][j]->Fill(dca2d * 10000.);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		for (int j = 0; j < 8; j++)
		{
			...
			hist[i][j] = new TH1F(Form("h_%i_%i", i, j), Form("Transverse DCA Resolution. #eta: %.1f - %.1f p: %d - %d GeV;#mum;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 100, -axis_range[j][i], axis_range[j][i]);
	...
	for (int j = 0; j < 8; j++)
	{
		...
		for (int i = 0; i < 6; i++){
			...
			c[j]->cd(i + 1);
			fit[j][i]->SetParameters(1, 1, fit_range, 1, 1, fit_range);
			fit[j][i]->SetParLimits(1, -10*(j+1), 10*(j+1));
			fit[j][i]->SetParLimits(4, -10*(j+1), 10*(j+1));
			fit[j][i]->SetParLimits(0, 0, hist[i][j]->GetMaximum()*1.1);
			fit[j][i]->SetParLimits(3, hist[i][j]->GetMaximum()*.05, hist[i][j]->GetMaximum()*1.1);
			fit[j][i]->SetParLimits(2, fit_range*1., 9999. );
			fit[j][i]->SetParLimits(5, fit_range*.2 ,fit_range*2.);
			hist[i][j]->Fit("fit", "R");	
			hist[i][j]->Draw();
