{
	...
	TH1F *dist = new TH1F("mom_dist", "Momentum Resolution distibution in momentum: p:counts", 50, -1000., 1000.);


	float test_axis = .05; //set this and plot so all data are in range	
	...
	for (int i=0; i<6; i++){	
		...
		for (int j=0; j<8; j++){	
			if (j<5) {
				...
				hist_range[i][j] = new TH1F(Form("h_range_%i_%i", i, j), Form("Momentum Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(p-gp)/gp;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 500, -test_axis, test_axis);
			}

			else {
				hist_range[i][j] = new TH1F(Form("h_range_%i_%i", i, j), Form("Momentum Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(p-gp)/gp;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 500, -test_axis*(j-2), test_axis*(j-2));
			}
		}
	}
	...
	for (int i = 0; i < 6; i++)
	{
		...
		for (int j = 0; j < 8; j++)
		{
			...
			hist[i][j] = new TH1F(Form("h_%i_%i", i, j), Form("Momentum Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(p-gp)/gp;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 100, -axis_range[j][i], axis_range[j][i]);
	...
	for (int j = 0; j < 8; j++)
	{
		...
		for (int i = 0; i < 6; i++){
			...
			fit[j][i] = new TF1("fit", "gaus(0)+gaus(3)", -fit_range*2.5, fit_range*2.5);

			c[j]->cd(i + 1);
			fit[j][i]->SetParameters(0, 0, fit_range, 0, 0, fit_range);
			fit[j][i]->SetParLimits(1, -.05*(j+1), .05*(j+1));
			fit[j][i]->SetParLimits(4, -.05*(j+1), .05*(j+1));
			fit[j][i]->SetParLimits(0, hist[i][j]->GetMaximum()*.05, hist[i][j]->GetMaximum()*0.4);
			fit[j][i]->SetParLimits(3, hist[i][j]->GetMaximum()*.1, hist[i][j]->GetMaximum()*1.1);
			fit[j][i]->SetParLimits(2, fit_range*.5, 1. );
			fit[j][i]->SetParLimits(5, fit_range*.01 ,fit_range*1.5);
			hist[i][j]->Fit("fit", "R");	
			hist[i][j]->Draw();
			...
			double std_dev2 = fit[j][i]->GetParameter(5);
			double std_err2 = fit[j][i]->GetParError(5);

			if (std_dev1 > std_dev2){

				sigmas[j][i] = std_dev2 * 100;
				std_errs[j][i] = std_err2 * 100;
			}
			else {

				sigmas[j][i] = std_dev1 * 100;
				std_errs[j][i] = std_err1 * 100;
			}
		}
	}
