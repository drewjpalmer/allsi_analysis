{
	...
	TString name1 = "out_simp_geom_sPHENIX_result_five_cone";
	TString str_input = name1 + ".root";
	TString str_out = "output_theta" + name1 + ".root";

	TFile *Fint = new TFile(str_input);

	TTree *myTree = (TTree *)Fint->Get("tracks");

	float px, py, pz, gpx, gpy, gpz, p, gp, eta, pt, gpt, geta, theta, gtheta, phi, gphi;
	...
	TH1F *dist = new TH1F("theta_dist", "Theta Resolution distibution in theta: radians:counts", 50, 0, 4);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		//binning by etw for fits 
		float axis_ends = 2*(.0005);
		float axis_middle = 2*(.0004 / i);
		...
		{
			//for Beast
			//float axis_ends = (0.0005 + (j*.0002) - (i * .00002));
			//float axis_middle = (0.0002 + (j*0.000025) - (i*.00002));
			//float axis = (.0005);
			if (i > 0 && i < 6)
			{

				hist[i][j] = new TH1F(Form("h_%i_%i", i, j), Form("Theta Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(theta_reconstructed - theta_generated) [rad];counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 50, -axis_middle, axis_middle);
			}
			else
			{

				hist[i][j] = new TH1F(Form("h_%i_%i", i, j), Form("Theta Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(theta_reconstructed - theta_generated) [rad];counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 50, -axis_ends, axis_ends);
			}
		}
	}
	...
	for (int entry = 0; entry < total_number_of_entries; entry++)
	{
		...
		if (px == px)
		{
			...
			geta = TMath::ATanH(gpz / gp);

			pt = sqrt(pow(px, 2) + pow(py, 2));
			gpt = sqrt(pow(gpx, 2) + pow(gpy, 2));
			theta = 2 * TMath::ATan(pow(TMath::E(), -eta)); 
			gtheta = 2 * TMath::ATan(pow(TMath::E(), -geta));
			phi = TMath::ASin(py / pt);
			gphi = TMath::ASin(gpy / gpt); 
	...
	for (int j = 0; j < 8; j++)
	{
		...
		for(int i = 0; i < 6; i++)
		{
			float fit_ends = 2*(0.0002 + (j * .000075));
			float fit_middle = 2*(.00015 - (i*.000025) + ((j+1)*.000005));

			//for Beast
			//float fit_ends = .75*(0.0005 + (j * .0002));
			//float fit_middle = .75*(0.0002 + (j * .0000286) - (i * 0.00002));
			//must change based on the field strength sigma ~ 1/B
			//for sPHENIX multiply everything by 2
			//binning in eta for theta resolution
