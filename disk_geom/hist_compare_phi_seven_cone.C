{
	...
	TCanvas *mom_canvas_b = new TCanvas("phi_canvas_b", "Phi resolution in p bins for 3.0T Beast map");
	mom_canvas_b->Divide(3,2);

	TCanvas *mom_canvas_s = new TCanvas("phi_canvas_s", "Phi resolution in p bins for 1.4T sPHENIX map");
	...
	for (int i = 0; i < 6; i++){
		...
		sigma1[i] = new TH1F(Form("Sigma1_h_%i", i), "Phi resolution1", 8, 0, 4); 
		sigma2[i] = new TH1F(Form("Sigma2_h_%i", i), "Phi resolution2", 8, 0, 4); 
		sigma3[i] = new TH1F(Form("Sigma3_h_%i", i), "Phi resolution3", 8, 0, 4); 
		sigma4[i] = new TH1F(Form("Sigma4_h_%i", i), "Phi resolution4", 8, 0, 4); 
		...
		sigmab[i] = (TH1F*)sigma3[i]->Clone(Form("_phi_res_b_%i", i));
		sigmab[i]->Divide(sigma1[i]);
		sigmab[i]->SetTitle(Form("7-disk/5-disk  p: %d - %d GeV;#eta;7-disk / 5-disk Phi Resolution", (i * 5), ((i * 5) + 5))); 
		...
		sigmas[i] = (TH1F*)sigma4[i]->Clone(Form("_phi_res_s_%i", i));
		sigmas[i]->Divide(sigma2[i]);
		sigmas[i]->SetTitle(Form("7-disk/5-disk  p: %d - %d GeV;#eta;7-disk / 5-disk Phi Resolution", (i * 5), ((i * 5) + 5))); 
