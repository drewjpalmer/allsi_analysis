{
	...
	TString name1 = "out_simp_geom_Beast_result_five_cone";
	TString str_input = name1 + ".root";
	TString str_out = "output_phi" + name1 + ".root";
	...
	TH1F *dist = new TH1F("phi_dist", "#phi Resolution distibution in theta: radians:counts", 50, 0, 4);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		{
			//for Beast
			float axis_ends = (0.0006 + (j*j*.0007) - (i * .0000));
			float axis_middle = (0.00025 + (j*j*0.0002) - (i*.00003));
			if (i > 0 && i < 6)
			{

				hist[i][j] = new TH1F(Form("h_%i_%i", i, j), Form("Phi Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(phi_reconstructed - phi_generated) [rad];counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 50, -axis_middle, axis_middle);
			}
			else
			{

				hist[i][j] = new TH1F(Form("h_%i_%i", i, j), Form("Phi Resolution. #eta: %.1f - %.1f p: %d - %d GeV;(phi_reconstructed - phi_generated) [rad];counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 50, -axis_ends, axis_ends);
			}
		}
	}
	...
	for (int j = 0; j < 8; j++)
	{
		...
		for(int i = 0; i < 6; i++)
		{
			...
			float fit_ends =.3*(0.0006 + (j*j*.0007) - (i * .0000));
			float fit_middle = .3*(0.00025 + (j*j*0.0002) - (i*.00003));
			if (i > 0 && i < 6)
			{

				fit[j] = new TF1("fit", "gaus", -fit_middle, fit_middle);
			}

			else
			{

				fit[j] = new TF1("fit", "gaus", -fit_ends, fit_ends);
			}
