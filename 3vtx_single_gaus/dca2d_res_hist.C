{
	...
	float test_axis = 40.; //set this and plot so all data are in range	
	...
	for (int i=0; i<6; i++){	
		...
		for (int j=0; j<8; j++){	
			...
			if (i<1)
			{
				hist_range[i][j] = new TH1F(Form("h_range_%i_%i", i, j), Form("Transverse DCA Resolution. #eta: %.1f - %.1f p: %d - %d GeV;#mum;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 500, -(j+1)*70., (j+1)*70.);
			}
			else
			{
				hist_range[i][j] = new TH1F(Form("h_range_%i_%i", i, j), Form("Transverse DCA Resolution. #eta: %.1f - %.1f p: %d - %d GeV;#mum;counts", (j) / 2., (j) / 2. + 0.5, (i * 5), ((i * 5) + 5)), 500, -test_axis*(j+4)/(i+1), test_axis*(j+4)/(i+1));
			}
		}
	}
			...
			for (int j = 0; j < 8; j++)
			{
				if (eta >= (float)(j) / 2. && eta < (float)(j) / 2. + 0.5)
				{
					for (int i = 0; i < 6; i++)
					{
						...
						axis_range[j][i] = fit_width[j][i]*3.;
					}
				}
			}
	...
	for (int j = 0; j < 8; j++)
	{
		...
		for (int i = 0; i < 6; i++){
			...
			if (i < 2)
			{
				fit[j][i] = new TF1("fit", "gaus(0)", -fit_range*.75, fit_range*.75);
			}
			else
			{
				fit[j][i] = new TF1("fit", "gaus(0)", -fit_range*1., fit_range*1.);
			}
